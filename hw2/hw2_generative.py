...
## NOTE model ... mu1.npy, mu0.npy, Sig.npy
...
## 4-4 Predictiom
feature_list = (['age','age_p2','age_p3'] + ['wc_c' + str(i) for i in range(2,7)] + ['education_num','education_num_p2'] + 
    ['mtstat_c' + str(i) for i in range(2,5)] + ['job_c' + str(i) for i in range(2,12)] + ['re_c' + str(i) for i in range(2,7)] + 
    ['race_c' + str(i) for i in range(2,5)] + ['sex_c2'] + ['country_c' + str(i) for i in range(2,5)] + ['has_capGain','has_capLoss','capFlow','capFlow_p2','capFlow_p3'] +
    ['hours_per_week','hours_per_week_p2','hours_per_week_p3'] + ['fnlwgt','fnlwgt_p2'] + ['Female_race_c' + str(i) for i in range(2,5)] + ['female_c_edu','female_c_edu_p2'] +
...
iSig = np.linalg.inv(Sig.T)
zval = lambda xvec: -0.5*((xvec-u1).dot(iSig).dot(xvec-u1) - (xvec-u0).dot(iSig).dot(xvec-u0)) + np.log(n1/n0)      
prob1 = np.array( [1/(1 + np.exp(-zval(X[i,]))) for i in range(X.shape[0])] )        
pred = (prob1 >= 0.5)*1


# exporting
filename = sys.argv[6]  #'../output/practice'
f1 = open(filename, 'w+')
